...
def bb_fast_IOU_v1(boxA, boxB):
    """
    计算交并比。Calculation of IOU, version numpy
    ...
    # 决定相交正方形的x,y坐标值。 determine the (x, y)-coordinates of the intersection rectangle
    xA = np.maximum(boxA[0], boxB[:, 0])
    yA = np.maximum(boxA[1], boxB[:, 1])
    xB = np.minimum(boxA[2], boxB[:, 2])
    yB = np.minimum(boxA[3], boxB[:, 3])

    # 计算相交正方形的面积。 compute the area of intersection rectangle
...
def mix_track_detV2(iou_mat, det, track):
    """
    根据检测结果精调跟踪框。refine track bounding boxes by detections
    :param iou_mat: 每个跟踪结果到每个检测结果的IOU距离矩阵。 iou between dets and tracks, numpy array, [num_track, num_det]
    :param det: 检测框。 detection bbox matrix, numpy array, [num_detections, 4]
    :param track: 跟踪框。 prediction from trackers, numpy array,  [num_tracks, 4]
    :return: refined new tracks, numpy array, [num_tracks, 4]
    """
    values, idx = torch.max(iou_mat, dim=1)  # 某个跟踪框到所有检测框中最大IOU，交并比越大表明越可能是这个跟踪目标的检测框
    mask = torch.ones_like(values)

    for i in range(iou_mat.shape[0]):  # 遍历每个跟踪框
        # iou < 0.6, no refinement
        if float(iou_mat[i, idx[i]]) <= 0.6:  # 某个跟踪框和所有检测框中没有一个的IOU大于0.6，则不需要更新跟踪框
            mask[i] = 0.0

    values = mask*values

    return (1.0-values).view(-1, 1)*track + values.view(-1, 1)*torch.index_select(det, 0, idx)  # 跟踪框和检测框的IOU越大(value越大)表示 检测结果(det)所占的重要性就越大
