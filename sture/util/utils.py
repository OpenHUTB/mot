        ...
        if cur_trajectory[1] - cur_trajectory[0] + 1 >= tracklet_len:
            ...
            for tracklet_start in tracklets_start:
                ...
                if same_pro > 0.5:  # positive sample
                    ...
                    labels[tracklet_idx] = torch.tensor(cur_trajectory[2])  # similarity(NAAN) --> person id(TKP)
                else:  # negative sample
                    ...
                    labels[tracklet_idx] = torch.tensor(cur_trajectory[2])
                det_idx = int(det_idx)
