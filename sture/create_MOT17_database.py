...
# --data_root /data/whd/MOT/ --dataset MOT17 --output_dir ./MOT17_database/
...
if __name__ == '__main__':
    ...
    train_root = args.data_root + args.dataset + '/train/'
    ...
    #     frames_gt = read_txt_gtV2(train_root + vname + '/gt/gt.txt')
    #     imgs_path = train_root + vname + '/img1/'
    ...
    #                                          ("%04d%02dF%04d.jpg") % (int(cur_gt_box[0]), int(vname[6:8]), frameid)),
    ...
    for vname in videos:
        ...
        for id in ids:
            images = sorted(os.listdir(os.path.join(video_dir, id)))
            if len(images) < 2:  # When len(images)==1, rand(start, end) error(end <= start)
                continue
            ...
            train_info[tracklet_idx, 3] = int(vname[6:8])  # camera id
            tracklet_idx += 1
            img_idx += len(images)
