...
# 运行
虚拟环境位于：（版本3.8太高，tensorflow为2.x，应该为1.x）
```
/home/d/.conda/envs/deploy/bin/python
```
...
```shell script
conda create -n mot_web python=3.6.12
conda activate mot_web
...
```shell script
cd src
python demo.py mot --load_model /data3/dong/data/mot/deploy/model/fairmot_dla34.pth --conf_thres 0.4
```
...
# 手机端
使用[IP摄像头](http://www.downcc.com/soft/340131.html)进行视频采集。
1. 打开IP摄像头APP的服务器（App最小面）；
2. 电脑端浏览器打开：
```shell script
http://10.69.119.210:8081/
```

[多个智能手机进行实时视频流式传输](https://leonlok.co.uk/blog/live-video-streaming-using-multiple-smartphones-with-imagezmq/)

[C++代码连接](https://blog.csdn.net/xiaoqiang_007_/article/details/106578900)

[Python拉取rtsp流](https://blog.csdn.net/fenghutu/article/details/116063233?utm_medium=distribute.pc_aggpage_search_result.none-task-blog-2~aggregatepage~first_rank_ecpm_v1~rank_v31_ecpm-1-116063233.pc_agg_new_rank&utm_term=python+%E8%BF%9E%E6%8E%A5%E6%B5%B7%E5%BA%B7%E6%91%84%E5%83%8F%E5%A4%B4rtsp&spm=1000.2123.3001.4430)

查看手机的8554端口是否打开：
```shell script
nmap 10.69.131.194 -p 8554
```
...
[原始代码仓库](https://github.com/LeonLok/Multi-Camera-Live-Object-Tracking) 
